...
export class SingleFile {
  ...
  private copyPiecesToContainer (
    ...
    for (const piece of this.scenario.pieces) {
      ...
      const isNoFile = piece.isNoFile as boolean
      ...
      if (isPieceStartingWithGoal1Met || pieceType.endsWith('SUB')) {
        ...
        if (isCopyRootPiecesOnly && isPieceStartingWithGoal1Met) {
          if (!isNoFile && goal1 !== '99_win') {
            ...
            if (!existsSync(path)) {
              throw new Error(
                `Ensure "isNoFile" is marked for goal ${goal1} of ${pieceType} in ${this.file}, because the following file doesn't exist ${path}`
              )
            }
