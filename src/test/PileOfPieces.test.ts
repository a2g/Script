...
describe('ReactionMap', () => {
  it('test AddToMap works', () => {
    ...
    // do it!
    blah.AddMapEntryUsingOutputAsKey(new Piece(0, 0, 'outputA', 'type', 1, null, null, 'A', 'B'))
  ...
  it('test RemoveNode works', () => {
    const blah = new PileOfPieces(null)
    for (let i = 0; i < 3; i += 1) {
      blah.AddMapEntryUsingOutputAsKey(new Piece(0, 0, 'outputA', 'piffle', 1, null, null, 'A', 'B'))
    }
