  ...
  GenerateSolutionNamesAndPush(mapOfStartingThingsAndWhoHasThem: Map<string, Set<string>>): void {
    for (let i = 0; i < this.solutions.length; i++) {
      ...
      for (let j = 0; j < this.solutions.length; j++) {
        ...
        const otherLeafs = otherSolution.GetRootNodeMap().GenerateMapOfLeaves()
        for (const leafNode of otherLeafs.values()) {
          ...
          if (result !== undefined) { otherLeafNodeNameCount = result }
      ...
      for (const leafNode of currLeaves.values()) {
        ...
        mapOfStartingThingsAndWhoHasThem.forEach((characters: Set<string>, key: string) => {
      ...
      currSolution.PushNameSegment('sol_' + minLeafNodeName + Colors.Reset + (accumulatedRestrictions.size > 0 ? AddBrackets(GetDisplayName(Array.from(accumulatedRestrictions))) : ''))
    }
  }
